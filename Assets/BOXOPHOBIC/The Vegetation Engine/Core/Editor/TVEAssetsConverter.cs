...
namespace TheVegetationEngine
{
    public class TVEAssetsConverter : EditorWindow
    {
        ...
        const string PREFABS_DATA_FOLDER = "/Prefabs Data/";
        ...
        {
            var originalPath = AssetDatabase.GetAssetPath(prefabObject);
            var originalFolder = Path.GetDirectoryName(originalPath);
            var prefabsFolder = originalFolder + PREFABS_DATA_FOLDER + "Backups";
            ...
            if (outputSuffix != "")
            {
                suffix = " (" + outputSuffix + " Backup)";
            }

            string savePath = prefabsFolder + "/" + prefabName + suffix + ".prefab";
        ...
        void ConvertMeshDefault(TVEGameObjectData gameObjectData)
        {
            ...
            for (int s = 0; s < subMeshCount; s++)
            {
                ...
                var material = gameObjectData.originalMaterials[s];
        ...
        void ConvertMeshNormals(TVEGameObjectData gameObjectData)
        {
            if (sourceNormals == 1)
            {
                ...
                for (int s = 0; s < subMeshCount; s++)
                {
                    ...
                    if (material != null && (material.shader.name.Contains("Bark") || material.shader.name.Contains("Prop")))
                    {
                        for (int i = baseVertex; i < endVertex; i++)
                        {
        ...
        string GetConvertedAssetName(Object asset, string type, bool shareCommonAssets)
        {
            ...
            if (outputSuffix != "")
            {
                suffix = " (" + outputSuffix + " " + type + ")";
            }

            if (outputGUIDsIndex == OutputGUIDs.Off)
            {
                assetName = name + extra + suffix;
            }
            else
            {
                assetName = name + extra + guid + suffix;
            }
        ...
        string GetConvertedAssetPath(Object asset, string name, string extention, string type)
        {
            ...
            string prefabsDataFolder = prefabFolder + PREFABS_DATA_FOLDER + type + "s";

            if (!Directory.Exists(prefabsDataFolder))
            {
                Directory.CreateDirectory(prefabsDataFolder);
                AssetDatabase.Refresh();
            }

            savePath = prefabsDataFolder + "/" + saveName;
        ...
        string GetCollectedAssetPath(Object asset, string type)
        {
            ...
            string prefabsDataFolder = collectDataFolder + PREFABS_DATA_FOLDER + type + "s";
        ...
        string GetPackedTexturePath(TVEPackerData packerData)
        {
            ...
            if (firstValidTexture != null)
            {
                ...
                if (outputSuffix != "")
                {
                    suffix = " (" + outputSuffix + " Texture)";
                }

                if (outputGUIDsIndex == OutputGUIDs.Off)
                {
                    saveName = firstValidTexture.name + suffix + extension;
                }
                else
                {
                    string guid = "";
                    ...
                    saveName = firstValidTexture.name + " " + guid + suffix + extension;
                }

                var prefabPath = AssetDatabase.GetAssetPath(prefabObject);
                var prefabFolder = Path.GetDirectoryName(prefabPath);

                string prefabsDataFolder = prefabFolder + PREFABS_DATA_FOLDER + "Textures";
