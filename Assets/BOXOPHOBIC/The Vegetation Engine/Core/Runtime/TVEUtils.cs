...
namespace TheVegetationEngine
{
    public class TVEUtils
    {
        ...
        public static void SetMaterialSettings(Material material)
        {
            ...
            // Set Blend Mode
            if (material.HasProperty("_TerrainMode") && material.HasProperty("_ThirdColor"))
            {
                if (material.GetInt("_TerrainMode") == 0)
                {
                    material.DisableKeyword("TVE_FEATURE_TERRAIN");
                }
                else
                {
                    material.EnableKeyword("TVE_FEATURE_TERRAIN");
                }

                if (material.HasProperty("_ThirdUVsMode"))
                {
                    var mode = material.GetInt("_ThirdUVsMode");

                    // Main
                    if (mode == 0)
                    {
                        material.SetVector("_third_uvs_mode", new Vector4(1, 0, 0, 0));
                    }
                    // Baked
                    else if (mode == 1)
                    {
                        material.SetVector("_third_uvs_mode", new Vector4(0, 1, 0, 0));
                    }
                    // Planar
                    else if (mode == 2)
                    {
                        material.SetVector("_third_uvs_mode", new Vector4(0, 0, 1, 0));
                    }
                    // Unused
                    else if (mode == 3)
                    {
                        material.SetVector("_third_uvs_mode", new Vector4(0, 0, 0, 1));
                    }
                }
            }
        ...
        public static Texture PackAndSaveTexture(string dataPath, TVEPackerData packerData, string outputSuffix)
        {
            ...
            if (outputSuffix != "")
            {
                AssetDatabase.SetLabels(AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(dataPath), new string[] { outputSuffix + " Texture" });
            }
        ...
        public static void SetShaderSettings(string shaderPath, TVEShaderSettings shaderSettings)
        {
            ...
            var cgincFLORA = "Packages/com.ma.flora/ShaderLibrary/Flora.hlsl";
