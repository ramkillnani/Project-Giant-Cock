...
public class LB_Terrain_2Editor : Editor 
{
	...
	public override void OnInspectorGUI()
	{
		...
		for (int a = 0; a < targetObject.splats.Length; a++) {
			if (GUILayout.Button (targetObject.splats[a].diffuseTexture, GUILayout.Width (43), GUILayout.Height (43)))
				currentLayer = a;
		}
		...
		var layerOffsetRef = targetObject.layerOffset[currentLayer];
		...
		targetObject.layerOffset[currentLayer] = EditorGUILayout.Slider("Layer Height Offset", targetObject.layerOffset[currentLayer], -10, 10);
		...
		if (uvRef != targetObject.uvS[currentLayer] || smoothnessValueRef != targetObject.smoothnessValue[currentLayer] || disValueRef != targetObject.disValue[currentLayer]
			|| normalPowerRef != targetObject.normalPower[currentLayer] || specularColorRef != targetObject.specularColor[currentLayer] || layerOffsetRef != targetObject.layerOffset[currentLayer]) {
